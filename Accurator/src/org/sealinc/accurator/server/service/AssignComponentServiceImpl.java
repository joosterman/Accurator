...
public class AssignComponentServiceImpl extends RemoteServiceServlet implements AssignComponentService {
	...
	public List<String> getNextItemsToAnnotate(int nritems,String matches) {
		String url = String.format("%s?output=%s&strategy=%s&nritems=%d&string=%s&predicate=http://purl.org/dc/terms/description",Config.assignComponentURL, "json",Config.assignComponentStrategy,nritems,matches);
		return Utility.getParsedJSONFromURL(url);
	}

	@Override
	public List<CollectionItem> search(String text) {
		String url = String.format("%s&string=%s",Config.recommendationComponentSearchURL,text);
